        ...
        for to_taz, vehicle_data in to_taz_data.items():
            for vehicle_type, count in vehicle_data.items():
                if vehicle_type == "moto":
                    ...
                    if (from_taz, to_taz) not in truck_data:
                        truck_data[(from_taz, to_taz)] = count
                    else:
                        truck_data[(from_taz, to_taz)] += count
                elif vehicle_type == "trailer":
                    if (from_taz, to_taz) not in trailer_data:
                        trailer_data[(from_taz, to_taz)] = count
                    else:
                        trailer_data[(from_taz, to_taz)] += count
    return moto_data, car_data, bus_data, truck_data, trailer_data
...
def main():
    city = "wroclaw"
    csv_file_path = f"data/{city}/daily_traffic_base.csv"
    print(csv_file_path)
    edges_data = open_csv_file(csv_file_path)
    traffic_allocation = allocate_traffic(edges_data)
    print(traffic_allocation)
    moto_data, car_data, bus_data, truck_data, trailer_data = generate_od_files(
        traffic_allocation
    )
    write_data_to_file(truck_data, "truck", city)
    write_data_to_file(trailer_data, "trailer", city)
