...
def main(options):
    ...
    def generate_one_person(
        label, combined_attrs, attrFrom, attrTo, departureTime, intermediate, options
    ):
        fouttrips.write(
    ...
    if "duarouter" in options_to_forward:
        for option in options_to_forward["duarouter"]:
            ...
            if option[0] not in args:
                ...
                raise ValueError(
                    "The argument '%s' has already been passed without the duarouter prefix."
                    % option[0]
                )
